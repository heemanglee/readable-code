...
public class MinesweeperGame {
    ...
    public static void main(String[] args) {
        ...
        while (true) {
            ...
            } catch(AppException e) { // 프로그램에서 발생하는 예외 처리
    ...
    private static void initializeGame() {
        ...
        for (int row = 0; row < BOARD_ROW_SIZE; row++) {
            for (int col = 0; col < BOARD_COL_SIZE; col++) {
                if (isLandMineCell(row, col)) { // 부정 연산자 제거
                    NEARBY_LAND_MINES_COUNTS[row][col] = 0;
                    continue;
                }
                int count = countNearByLandMines(row, col); // 추상화
    ...
    private static void open(int row, int col) {
        ...
        if (BOARD[row][col].doesNotEqualsSign(CLOSED_CELL_SIGN)) {
            return;
        }
        ...
        if (NEARBY_LAND_MINES_COUNTS[row][col] != 0) {
            BOARD[row][col] = Cell.of(String.valueOf(NEARBY_LAND_MINES_COUNTS[row][col]));
            return;
        } else {
